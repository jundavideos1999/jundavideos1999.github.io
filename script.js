...
window.addEventListener('load', () => {
    ...
    const introSource = document.getElementById('intro-video-source');
    const hero = document.getElementById('hero-bg-video');

    // 根據設備類型設定影片源
    if (isMobile() && introSource) {
        introSource.src = 'videos/Render in 4K_phone.mp4';
        // 重新載入影片源
        intro.load();
    }
    ...
    const hideLoaderAndStartHero = () => {
        ...
        if (hero) {
            console.log('LOGO 影片結束，嘗試播放背景影片');
            // 確保影片準備好後再播放
            const tryPlayHero = () => {
                console.log('嘗試播放背景影片，readyState:', hero.readyState);
                const playPromise = hero.play();
                if (playPromise && typeof playPromise.then === 'function') {
                    playPromise.then(() => {
                        console.log('背景影片播放成功');
                        heroVideoPlayed = true;
                    }).catch((error) => {
                        console.log('背景影片自動播放被阻擋:', error);
                        // 在手機端，嘗試用戶交互觸發播放
                        if (isMobile()) {
                            console.log('手機端背景影片需要用戶交互才能播放，設置監聽器');
                            // 監聽用戶交互來觸發背景影片播放
                            ['touchstart', 'touchend', 'click'].forEach(eventType => {
                                document.addEventListener(eventType, enableHeroVideoPlayback, { once: true });
                            });
                        }
                    });
                } else {
                    console.log('背景影片立即播放');
                    heroVideoPlayed = true;
                }
            };
            ...
            if (hero.readyState >= 3) {
                ...
                hero.addEventListener('canplaythrough', tryPlayHero, { once: true });
                hero.addEventListener('loadeddata', tryPlayHero, { once: true });
            }
        }
    };
    ...
    if (intro) {
        ...
        const tryPlayIntro = () => {
            ...
            if (playPromise && typeof playPromise.then === 'function') {
                playPromise.catch((error) => {
                    console.log('影片自動播放被阻擋，使用保險計時結束:', error);
                    // 自動播放被阻擋時，改用保險計時結束
                });
            }
        };
        ...
        if (intro.readyState >= 3) {
            ...
            // 額外監聽 loadeddata 事件，確保影片數據載入完成
            intro.addEventListener('loadeddata', tryPlayIntro, { once: true });
        }
    } else {
...
// 手機端背景影片播放處理 - 簡化版本，與電腦端保持一致
let heroVideoPlayed = false;
const enableHeroVideoPlayback = () => {
    if (heroVideoPlayed) return;
    
    const hero = document.getElementById('hero-bg-video');
    console.log('嘗試播放背景影片，影片元素:', hero);
    ...
    } else if (hero && !hero.paused) {
        console.log('背景影片已經在播放');
        heroVideoPlayed = true;
    }
};
